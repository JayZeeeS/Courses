...
#reader(lib "htdp-beginner-abbr-reader.ss" "lang")((modname Naturals-and-Helpers-Design-Quiz-starter) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
(require 2htdp/image)
...
;; ======================================================================
;; Constants
(define COOKIES (triangle 10 "solid" "gold"))
...
;; Natural Image -> Image
;; place an image besides itself n times
(check-expect (pyramid-slice 0 COOKIES) empty-image)
(check-expect (pyramid-slice 3 COOKIES) (beside COOKIES COOKIES COOKIES empty-image))

;(define (pyramid-slice n img) empty-image) ;stub

(define (pyramid-slice n img)
  (cond [(zero? n) empty-image]
        [else
         (beside img
                 (pyramid-slice (sub1 n) img))]))
...
;; produce a modified version of the given list where the given blob rises up (gets inseted before the list)
;; if it is a bubble, otherwise it sinks down (gets inserted after the first element).
(check-expect (percolate "solid" empty) (cons "solid" empty))
(check-expect (percolate "solid" (cons "bubble" empty)) (cons "bubble" (cons "solid" empty)))
(check-expect (percolate "bubble" (cons "solid" empty)) (cons "bubble" (cons "solid" empty)))

;(define (percolate b lob) lob) ;stub

(define (percolate b lob)
